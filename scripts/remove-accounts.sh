#!/system/bin/sh
# Sourced from fcjr/unmetaportal, tools/portal-remove-facebook-accounts.sh
# Push to /data/local/tmp and run via `adb shell sh`.
# Uses CVE-2024-31317 (Zygote command injection) to spawn a process as the
# alohausers UID, then calls AccountManagerService.removeAccount for every
# com.facebook.aloha.* account found via `dumpsys account`.
set -eu
